...
dir_result <- "C:/A_TRABAJO/A_GABRIEL/REPRESENTATIVIDAD/TEST_3/"
...
for(j in 4:length(mh)){
  ...
  writeRaster(raster, paste0(dir_result, "PRE_sig_", names[j - 3], ".tif"), overwrite =
                TRUE)
...
for(j in 4:length(mh)){
  ...
  raster <- terra::rasterize(puntos_vect, raster_template, field = "lisa_cluster")
  crs(raster) <- crs(mh_raster)
  writeRaster(raster, paste0(dir_result, "SSP585_2070_", names[j - 3], ".tif"), overwrite =
                TRUE)
  parches <- terra::patches(raster, directions = 8, zeroAsNA=T)
  crs(parches) <- crs(raster)
  parche_pt <- terra::extract(parches, puntos_todos)
  ...
  raster <- terra::rasterize(puntos_vect, raster_template, field = "parche")
  crs(raster) <- crs(mh_raster)
  writeRaster(raster, paste0(dir_result, "SSP585_2070_patch_", names[j - 3], ".tif"), overwrite =
                TRUE)
  plot(raster)
  raster <- terra::rasterize(puntos_vect, raster_template, field = "SA")
  crs(raster) <- crs(mh_raster)
  writeRaster(raster, paste0(dir_result, "SSP585_2070_SA_", names[j - 3], ".tif"), overwrite =
                TRUE)
  }
