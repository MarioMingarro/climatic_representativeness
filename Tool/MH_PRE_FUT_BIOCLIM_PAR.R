...
dir_result <- "C:/A_TRABAJO/A_GABRIEL/REPRESENTATIVIDAD/TEST_KBA2/RES/"

study_area <- read_sf("C:/A_TRABAJO/A_GABRIEL/REPRESENTATIVIDAD/Peninsula_Iberica_89.shp")
polygon <- read_sf("C:/A_TRABAJO/A_GABRIEL/REPRESENTATIVIDAD/TEST_KBA2/KBA_test.shp")
...
process_present <- function(j) {
  ...
  puntos_todos$lisa_cluster <- case_when(
    puntos_todos$SA_sig >= 0.01 ~ NA,
    puntos_todos$mh < max(puntos_dentro$mh) & puntos_todos$SA > 0 ~ 1,
  ...
  writeRaster(raster, paste0(dir_result, "PRE_sig_", names[j - 3], ".tif"), overwrite = TRUE)
  
  parches <- terra::patches(raster, directions = 8, zeroAsNA = TRUE)
  crs(parches) <- crs(raster)
  parche_pt <- terra::extract(parches, puntos_todos)
  puntos_todos <- puntos_todos %>% mutate(parche = parche_pt$patches)
  puntos_vect <- vect(puntos_todos)
  
  raster <- terra::rasterize(puntos_vect, raster_template, field = "parche")
  crs(raster) <- crs(mh_raster)
  writeRaster(raster, paste0(dir_result, "PRE_patch_", names[j - 3], ".tif"), overwrite = TRUE)
  plot(raster)
  
  raster <- terra::rasterize(puntos_vect, raster_template, field = "SA")
  crs(raster) <- crs(mh_raster)
  writeRaster(raster, paste0(dir_result, "PRE_SA_", names[j - 3], ".tif"), overwrite = TRUE)
}
...
foreach(j = 4:length(mh), .packages = c("terra", "sf", "dplyr", "spdep")) %dopar% {
  process_present(j)
}
...
process_future <- function(j) {
  ...
  mh_raster <- terra::rast(mh_raster[, c(1:2, j)], crs = reference_system)
  names(mh_raster) <- colnames(mh[j])
  writeRaster(mh_raster, paste0(dir_result, "SSP585_2070_", names[j-3], ".tif"), overwrite = TRUE)
  ...
  raster <- terra::rasterize(puntos_vect, raster_template, field = "lisa_cluster")
  crs(raster) <- crs(mh_raster)
  writeRaster(raster, paste0(dir_result, "SSP585_2070_sig_", names[j - 3], ".tif"), overwrite = TRUE)
  ...
  raster <- terra::rasterize(puntos_vect, raster_template, field = "parche")
  crs(raster) <- crs(mh_raster)
  writeRaster(raster, paste0(dir_result, "SSP585_2070_patch_", names[j - 3], ".tif"), overwrite = TRUE)
  plot(raster)
  
  raster <- terra::rasterize(puntos_vect, raster_template, field = "SA")
  crs(raster) <- crs(mh_raster)
  writeRaster(raster, paste0(dir_result, "SSP585_2070_SA_", names[j - 3], ".tif"), overwrite = TRUE)
}
